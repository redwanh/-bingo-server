...
socket.on('buyCard', async (data, callback) => {
  try {
    const { roomId, cardId } = data;
    const userId = socket.userId;

    console.log(`🛒 [BUY] ${socket.username} buying card: ${cardId}`);
    ...
    if (!config) {
      return callback({ success: false, error: 'No config found' });
    }
    ...
    if (game.timerStartedAt && game.timerDuration) {
      ...
      if (Date.now() >= timerEnd) {
        return callback({ success: false, error: 'Registration time has ended' });
      }
    }

    // Check max cards
    const myCardCount = await Card.countDocuments({ gameId: game._id, userId, status: 'registered' });
    if (myCardCount >= config.maxCardsPerPlayer) {
      return callback({ success: false, error: `Maximum ${config.maxCardsPerPlayer} cards allowed` });
    }

    // Find the card
    const card = await Card.findOne({ _id: cardId, gameId: game._id, status: 'preview' });
    if (!card) {
      return callback({ success: false, error: 'Card not available' });
    }
    ...
    user.walletBalance = balanceBefore - price;
    await user.save();
    ...
    card.cardNumber = (game.totalCards || 0) + 1;
    await card.save();

    // Update game
    game.totalCards = (game.totalCards || 0) + 1;
    game.prizePool = (game.prizePool || 0) + price;

    if (!game.players?.find(p => p.userId?.toString() === userId)) {
      game.players = game.players || [];
      game.players.push({ userId, cards: [card._id] });
      game.playerCount = game.players.length;
    } else {
      const player = game.players.find(p => p.userId?.toString() === userId);
      if (player) player.cards.push(card._id);
    }
    await game.save();
    ...
    await Transaction.create({
      ...
      description: `Card #${card.displayId - 10000} purchased`,
      performedBy: user._id,
      performedByRole: 'user',
      metadata: { cardId: card._id, displayId: card.displayId },
    });
    ...
    console.log(`✅ [BUY] Card purchased by ${socket.username}`);
